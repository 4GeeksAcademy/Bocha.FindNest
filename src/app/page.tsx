...
import { type Listing, type SearchFilters } from '@/types';
...
import { buildSearchHref, parseSearchFilters, matchesDestination } from '@/lib/search';

export default function HomePage() {
  return (
    <Suspense fallback={<LoadingSpinner label="Preparando búsqueda..." />}>
      <HomePageContent />
    </Suspense>
  );
}
...
function HomePageContent() {
  ...
  const filtered = listings.filter((l) => {
    return matchesDestination(l, searchFilters.destination)
      ...
      <SiteHeader
        right={
          <div className="flex items-center gap-2">
            <Link href={buildSearchHref('/catalog', searchFilters)} className="hidden md:block text-sm font-semibold px-4 py-2 rounded-full hover:bg-gray-100 transition-colors whitespace-nowrap">
              Ver catálogo
            </Link>
